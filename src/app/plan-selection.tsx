...
const PLANS = [
    {
        type: 'Arcade' as PlantType,
        price: {monthly: 9.90, yearly: 99.90},
        icon: GamepadIcon,
        color: 'bg-orange-100 text-orange-500'
    },
    {
        type: 'Advanced' as PlantType,
        price: {monthly: 12.90, yearly: 129.90},
        icon: MonitorIcon,
        color: 'bg-pink-100 text-pink-500'
    },
    {
        type: 'Pro' as PlantType,
        price: {monthly: 15.90, yearly: 159.90},
        icon: TrophyIcon,
...
export function PlanSelection(){
    const {state, setPlan, setStep, setBillingCycle} = useSubscription();
    ...
    return(
        <div className="space-y-6">
                    ...
                    return(
                        ...
                        <div className="text-left">
                            ...
                            <div className="text-gray-500">
                                ${state.billingCycle === 'monthly' ? `${plan.price.monthly}/mo` : `${plan.price.yearly}/yr`}
                            </div>
            ...
            <div className="flex items-center justify-center space-x-4 py-4 bg-gray-50 rounded-lg">
                <span className={state.billingCycle === 'monthly' ? 'font-medium' : 'text-gray-500'}>Monthly</span>
                <Switch
                    checked={state.billingCycle === 'yearly'}
                    onChange={(checked) => setBillingCycle(checked ? 'yearly' : 'monthly')}
                />
                <span className={state.billingCycle === 'yearly' ? 'font-medium' : 'text-gray-500'}>Yearly</span>
            </div>
